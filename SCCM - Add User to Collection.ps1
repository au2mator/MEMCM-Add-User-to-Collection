...
# Type: New Service
#
# Title: SCCM - Add User to Collection
#
...
#
# URL: 
# Github: click.au2mator.com/MEMCM_AddUserToCollection
#################
...
try {
    Write-au2matorLog -Type INFO -Text "Find  User in SCCM"
    $CMUser=Get-WmiObject -ComputerName $SiteServer -Namespace "root\SMS\site_$CMSiteCode" -Class "SMS_R_User" -Filter "DistinguishedName = '$($($c_User).replace('\','\\'))'" | select name,resourceid
    Write-au2matorLog -Type INFO -Text "Found the User from CM: $($CMUser.ResourceID)"  

    
    try {
        Write-au2matorLog -Type INFO -Text "Try to Add User to Collection $($coll.Name)"
        ...
        [array]$rules = $null
                ...
                $newRule.RuleName = $($CMUser.name)
                $newRule.ResourceClassName = "SMS_R_User"	
                $newRule.ResourceID = $($CMUser.resourceid)
                $rules += $newRule
        ...
        $au2matorReturn = "User $($CMUser.name) added to Collection $($coll.Name)"
        $AdditionalHTML = "User $($CMUser.name) added to Collection $($coll.Name)
            <br>
                "
        $Status = "COMPLETED"
        ...
        Write-au2matorLog -Type ERROR -Text "Error on adding to User to Collection, Error: $Error"       
    
        $au2matorReturn = "Error on adding to User to Collection, Error: $Error"
        $AdditionalHTML = "Error on adding to User to Collection
            <br>
            Error: $Error
                "
        $Status = "ERROR"
